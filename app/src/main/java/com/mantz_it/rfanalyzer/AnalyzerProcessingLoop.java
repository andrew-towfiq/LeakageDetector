...
public class AnalyzerProcessingLoop extends Thread {
	...
	private float interestingInterval=600000.0f;// Interesting interval where we will be looking for energy in Hz.
	private int deltaIndex;						// delta index for filter function;
	private float power = 0;					// Power within our interesting interval
	...
	/*	try {
			SamplePacket samplePacket = inputQueue.poll(1000 / frameRate, TimeUnit.MILLISECONDS);
			float samplesPerHz = (float) fftSize/ (float) samplePacket.getSampleRate();
			deltaIndex = Math.round(interestingInterval*samplesPerHz/2);
	...
	/**
	 * Initialize Power filter (deltaIndex) for power power calculation based on fftSize and sampleSize
	 * @param sampleRate - sample size
	 */
	void initializePowerFilter(int sampleRate) {
		float samplesPerHz = (float) fftSize/ (float) sampleRate;  // spectral resolution in samples/Hz
		deltaIndex = Math.round(interestingInterval*samplesPerHz/2); // ±1/2 of interesting interval * samplesPerHz
	}
	...
	public void run() {
		...
		while(!stopRequested) {
			...
			initializePowerFilter(sampleRate);
			...
			view.draw(mag, frequency, sampleRate, frameRate, load, power);
	...
	public void doProcessing(SamplePacket samples) {
		...
		for (int i = 0; i < size; i++) {
			...
			float m = realPower + imagPower;
			mag[targetIndex] = (float) (10* Math.log10(Math.sqrt(m)));
			if(isInRangeForPowerFilter(targetIndex)) {
				power += m;
			}
		}
		power = (float) (10.0*Math.log10(power));
	}

	/**
	 * Power filter checking function
	 * @param targetIndex - magnitude index [0..fftsize)
	 * @return if index is in rage for power filter magnitude calculation
	 */
	boolean isInRangeForPowerFilter(int targetIndex){
		return (targetIndex > fftSize-deltaIndex) && (targetIndex < fftSize+deltaIndex);
	}
}
